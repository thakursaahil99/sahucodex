...
async def test_ai_usage_summary_reflects_real_requests(build_app) -> None:
    ...
    provider_down = FakeAiProvider(down=True)
    app2 = await build_app(settings=make_settings(**CONFIGURED), ai_provider=provider_down)
    async with signed_in(app2, username="bob") as (client2, headers2):
    ...
    assert chat["requests"] == 1 and chat["failed"] == 0
